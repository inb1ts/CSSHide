...
class CSSGenerator:
    ...
    def random_attrib_singular(self, color) -> str:
        method = random.choice([self.gen_color, self.gen_background_color, self.gen_background, self.gen_border, self.gen_outline])
        return method(color)
    ...
    def gen_variables_section(self, variable_colors) -> list[str]:
        ...
        while(True):
            ...
            for attr_index in shuffled_elements[element_names[i]]:
                ...
                variables_output.append(f"--{element_names[i]}-{attr_index}-color: {color}")
                variable_counter += 1
            i += 1
